// TODO При загрузке страницы нам высвечивается 1 модальное окно(prompt), где нас просят ввести локаль(RU, EN или DE).
//  Ответ записываем в cookie. При последующей загрузке страницы, если данные в cookie имеются, то не спрашиваем.
//  Теперь будем брать данные из cookie

// TODO В соответствии с выбранной локалью отправляем введенное значение как ключ на сервер, чтобы получить нужный перевод для стран и городов.

// TODO Наш полученный объект сохраняем в localStorage. Это необходимо, чтобы при обновлении страницы больше
//  не отправлять запрос на сервер. Данные уже будем брать из локального хранилища и загружать в верстку.

// TODO В зависимости оттого, какую локаль выбрали, то такую страну и ставим вверх списка. Выбрали DE, значит Германия должна быть первой в нашем списке.
